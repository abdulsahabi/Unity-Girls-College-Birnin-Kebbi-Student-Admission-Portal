  ...
  if (!empty($verifyCode)) {
    if ($user["verify_digit"] == $verifyCode) {
      ...
      if ($expireIn <= $expireMin) {
        $verified = $pdo->prepare(VERIFY);
        $verified->execute([1, $can_id]);
      ...
      echo json_encode([
        "error" =>
          "Sorry, the verification code you entered doesn't match. Please try again, and we'll get you verified in no time!",
      ]);
    }
  } else {
    header("Content-Type: application/json");
